...
def toIPA(string):
    ...
    if ortho is None:
        ...
        noOrthBox.setIcon(QtWidgets.QMessageBox.Icon.Warning)
        noOrthBox.setText("No primary orthography")
        noOrthBox.setInformativeText('Set a primary orthography on the Metadata tab.')
        noOrthBox.setStandardButtons(QtWidgets.QMessageBox.StandardButton.Ok)
        noOrthBox.setDefaultButton(QtWidgets.QMessageBox.StandardButton.Ok)
        noOrthBox.exec()
        return
...
def changeDisplayOrthography(fldbk, index, cardtype):
    if cardtype == 'Txt':
    ...
    if dataIndex.displayOrthography != comboBox.currentText():
        ...
        if comboBox.currentText() == 'Phonetic':
            ...
            if cardtype == 'Ex':
                ...
                for table in fldbk.tText.children():
                    if table.objectName() != 'textLayout':
                        changeTextDisplay(pairList, table)
        else:
            ...
            if cardtype == 'Ex':
                ...
                for table in fldbk.tText.children():
                    if table.objectName() != 'textLayout':
                        reverseTextDisplay(pairList, table)
...
def changeTextDisplay(pairList, table):
    try:
...
def reverseTextDisplay(pairList, table):
    try:
        string = table.item(0, 0).text()
