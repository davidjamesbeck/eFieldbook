...
from ELFB import dataIndex, HTMLDelegate, dsetOnlyBtns, formattingHandlers
from ELFB.palettes import EgSearchDialog,  TextSearchDialog, DSetSearchDialog
import operator
...
class SearchEngine(QtCore.QObject):
    ...
    def searchElement(self, entry, term, target):
        ...
        if self.parameters[2] == 0:  #parameters[2] flags caps
            p = QtCore.QRegularExpression(lookFor, QtCore.QRegularExpression.CaseInsensitiveOption)
        else:
            p = QtCore.QRegularExpression(lookFor, QtCore.QRegularExpression.CaseInsensitiveOption)
...
class LexSearchEngine(SearchEngine):
    ...
    def setScope(self):
        ...
        userDict = {'lSearchSource' : 'Speaker', 'lSearchResearcher' : 'Researcher', 'lSearchDate' : 'Date', 
                        'lSearchConfirmed' : 'Confirmed', 'lSearchUpdate' : 'Update',  
        ...
        try:
            ...
            if term[0] == '@L1Index':
                ...
                if hit == False:
                    ...
                    for node in defList:
                        if node.attrib.get('L1Index') != None:
            ...
            if term[0] == '@L2Index':
                ...
                if hit == False:
                    defList = entry.findall('Def')
                    for node in defList:
                        if node.attrib.get('L2Index') != None:
        ...
        if self.list == None:
            ...
            for entry in dataIndex.root.iter('Lex'):
                ...
                for term in searchList:
                    ...
                    if hit == False and self.orSwitch == 'off':
                        #if X is not in Y AND we need X in both, search next entry
                        break
                    elif hit == True and self.orSwitch == 'on':
...
class DSetSearchEngine(SearchEngine):
    ...
    def findInField(self, lookFor, parameters):
        ...
        if "#" in lookFor:
            lookFor = self.setEdge(lookFor)
        if parameters[2] == 0:
            regex = QtCore.QRegularExpression(lookFor, QtCore.QRegularExpression.CaseInsensitiveOption)
        else:
