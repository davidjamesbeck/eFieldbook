...
from os import path
from ELFB import Orthographies, dataIndex,  Alphabetizer, dictBuilder, idGenerator
...
def oSet(fldbk):
    ...
    if dataIndex.root.get('Orth'):
        ...
        if fldbk.oList.item(row, 0).text() == dataIndex.root.get('Orth'):
            ...
            breakbox.exec_()
            if breakbox.result() == QtWidgets.QMessageBox.Cancel:    
                return
...
def oNew(fldbk):
    ...
    if newName[1] != False and len(newName[0]) > 0:
        kind = 'export'
        root = dataIndex.root
        elemList = list(root)
        elemList.reverse()
        ...
        if fldbk.oList.rowCount() == 0:
            ...
            newPrimeBox.setIcon(QtWidgets.QMessageBox.Question)
            newPrimeBox.setStandardButtons(QtWidgets.QMessageBox.Cancel | QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No)
            newPrimeBox.setDefaultButton(QtWidgets.QMessageBox.Yes)
...
def mRDataCheck(fldbk, rCode):
    ...
    for i in range(0,fldbk.mRTable.rowCount()):
        if fldbk.mRTable.item(i,0).text() == rCode:
            ...
            duplicateCodeBox.setIcon(QtWidgets.QMessageBox.Warning)
            duplicateCodeBox.setStandardButtons(QtWidgets.QMessageBox.Ok)
            duplicateCodeBox.setDefaultButton(QtWidgets.QMessageBox.Ok)
...
def mRAdd(fldbk):
    ...
    for j in range(0,fldbk.mRTable.columnCount()-1):
        fldbk.mRTable.resizeColumnToContents(j)
    fldbk.mRTable.sortItems(0,QtCore.Qt.AscendingOrder)
    fldbk.mRTable.scrollToItem(newItem, QtWidgets.QAbstractItemView.PositionAtCenter)
...
def mRDel(fldbk):
    ...
    deletedCodeBox.setIcon(QtWidgets.QMessageBox.Critical)
    purgeBtn = deletedCodeBox.addButton("Purge",QtWidgets.QMessageBox.ActionRole)
    purgeBtn.setToolTip("remove all instances of this code from\n database and replace with “YYY”.")
    deletedCodeBox.setStandardButtons(QtWidgets.QMessageBox.Cancel | QtWidgets.QMessageBox.Ok)
    deletedCodeBox.setDefaultButton(QtWidgets.QMessageBox.Ok)
...
def mSpAdd(fldbk):
    ...
    fldbk.mSpTable.sortItems(0,QtCore.Qt.AscendingOrder)
    fldbk.mSpTable.scrollToItem(newItem, QtWidgets.QAbstractItemView.PositionAtCenter)
