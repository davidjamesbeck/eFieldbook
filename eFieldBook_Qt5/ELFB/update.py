...
def setContents(fldbk, fieldname):
    ...
    if fieldname == 'lOrthography':
        ...
        if newContent != oldListText:
            ...
            if child.attrib.get('Hom') is not None:
    ...
    if fieldname == 'lDialect':
        ...
        crossRef = None
        oldCrossRef = None
        if child.find('Dia') is not None:
            oldAlt = child.findall('Dia/Alternative')
            oldCrossRef = []
            if oldAlt is not None:
                for item in oldAlt:
        ...
        if newContent:
            ...
            if altList is not None and len(altList) != 0:
                crossRefList = []
                alterList = []
                for j in range(0, len(altList)):
                    ...
                    for entry in dataIndex.root.iter('Lex'):
                        ...
                        if lexeme == alternate and entry.attrib.get('Hom') is not None:
    ...
    if fieldname == 'lUpdated':
        newContent = fldbk.lUpdated.toPlainText()
        child.set('Update', newContent)

    if fieldname == 'lConfirmed':
    ...
    if fieldname == 'eLine':
        ...
        if dataIndex.currentText is not None and ExNode.attrib.get('SourceText') is not None:
            prepareTextUpdate(fldbk, ExNode)

    if fieldname == 'eL1Gloss':
    ...
    if fieldname == 'tTitle':
        ...
        fldbk.tTextNav.model().setSortCaseSensitivity(QtCore.Qt.CaseSensitivity.CaseInsensitive)
        fldbk.tTextNav.model().sort(0, QtCore.Qt.SortOrder.AscendingOrder)
        fldbk.tTextNav.scrollTo(fldbk.tTextNav.currentIndex(), QtWidgets.QAbstractItemView.ScrollHint.EnsureVisible)

    """Datasets"""
    if fieldname == 'dSource':
        newContent = fldbk.dSource.toPlainText()
        child.set('Spkr', newContent)

    if fieldname == 'dResearcher':
    ...
    if fieldname == 'dTitle':
        ...
        fldbk.dDataNav.model().setSortCaseSensitivity(QtCore.Qt.CaseSensitivity.CaseInsensitive)
        fldbk.dDataNav.model().sort(0, QtCore.Qt.SortOrder.AscendingOrder)
        fldbk.dDataNav.scrollTo(fldbk.dDataNav.currentIndex(), QtWidgets.QAbstractItemView.ScrollHint.EnsureVisible)
    ...
    if fieldname == 'sOrder' or fieldname == 'sExclusions':
