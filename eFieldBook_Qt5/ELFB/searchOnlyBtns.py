...
def load(fldbk):
    ...
    loadDialog.setFileMode(QtWidgets.QFileDialog.FileMode.ExistingFile)
    loadDialog.setOption(QtWidgets.QFileDialog.Option.ReadOnly)
    if loadDialog.exec():
        fname = loadDialog.selectedFiles()[0]
        openarchive = open(fname, 'r', encoding='UTF-8').read()
    else:
        return
    hitsList = openarchive.split('; ')
...
def searchBrowser(fldbk):
    if fldbk.cSearchResults.model() is not None:
        try:
