...
class ElanImporter(QtWidgets.QDialog, Ui_ElanImporter):
    ...
    def __init__(self, parent=None):
        ...
        self.targetList.horizontalHeader().setSectionResizeMode(0, QtWidgets.QHeaderView.Stretch)
    ...
    def displayTiers(self):
        ...
        for node in timeOrder.iter('TIME_SLOT'):
            if node.attrib.get('TIME_VALUE') != None:
    ...
    def getFilePath(self):
        ...
        fileDialog.setOption(QtWidgets.QFileDialog.ReadOnly)  
        if fileDialog.exec_():
    ...
    def gatherNodes(self, child, parent=None):
        ...
        for node in tier.iter('ANNOTATION'):
            ...
            id = alignable.attrib.get('ANNOTATION_ID')
            value = alignable.find('ANNOTATION_VALUE')
            contents = value.text
            element.set('ANNOTATION_ID', id)
            self.elanAnnotationIdDict[id] = element
            if alignable.attrib.get('TIME_SLOT_REF1') != None:
                try:
                ...
                try:
                    ...
                    pass
            if alignable.attrib.get('ANNOTATION_REF') != None:
        ...
        if parent == None:
        ...
        elif elemList[0].attrib.get('start') != None: #is the tier time-aligned rather than linked
            for upperNode in self.text.iter(parent2):
                ...
                try: 
                    ...
                    break
                i += 1
                while elemList[0].attrib.get('start') == None:
                    upperNode.insert(i, elemList.pop(0))
                    try: 
                        if elemList[0].attrib.get('start') != None:
                            if upperNode.attrib.get('start') < elemList[0].attrib.get('start') < upperNode.attrib.get('end'):
    ...
    def importText(self):
        ...
        newText = ''
        timeCodeList = ''
        if self.targetList.item(0, 0) == None:
            return
        ...
        if len(errorList) != 0:
            ...
            errorWarning.setIcon(QtWidgets.QMessageBox.Warning)
            ...
            errorWarning.setStandardButtons(QtWidgets.QMessageBox.Save|QtWidgets.QMessageBox.Cancel)
            errorWarning.setDefaultButton(QtWidgets.QMessageBox.Save)
            if errorWarning.exec_() == QtWidgets.QMessageBox.Save:
