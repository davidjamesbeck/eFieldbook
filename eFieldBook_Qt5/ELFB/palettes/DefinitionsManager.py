...
class DefinitionsManager(QtWidgets.QDialog, Ui_Dialog):
    ...
    def __init__(self, parent):
        ...
        self.setupUi(self)
        italics = QtWidgets.QAction(self)
        self.addAction(italics)
        italics.setShortcut(QtGui.QKeySequence.Italic)
        italics.triggered.connect(self.italics)
        underline = QtWidgets.QAction(self)
        self.addAction(underline)
        underline.setShortcut(QtGui.QKeySequence.Underline)
        underline.triggered.connect(self.underline)
        bold = QtWidgets.QAction(self)
        self.addAction(bold)
        bold.setShortcut(QtGui.QKeySequence.Bold)
        bold.triggered.connect(self.bold) 
        normal = QtWidgets.QAction(self)
        self.addAction(normal)
        normal.setShortcut(QtGui.QKeySequence(QtCore.Qt.CTRL + QtCore.Qt.SHIFT + QtCore.Qt.Key_I))
        ...
        if child.find('Def/L1').text == None:
        ...
        self.POS.setToolTip(QtWidgets.QApplication.translate("DefinitionsManager", "part of speech (if different <br />for this subentry)", None))
        ...
        self.Reg.setToolTip(QtWidgets.QApplication.translate("DefinitionsManager", "register (if specific <br />to this subentry)", None))
        ...
        self.Dia.setToolTip(QtWidgets.QApplication.translate("DefinitionsManager", "dialect (if specific <br />to this subentry)", None))
        ...
        self.Alternative.setToolTip(QtWidgets.QApplication.translate("DefinitionsManager", "alternative form in other dialects.<br /> "
                                                                 "Use the format: US. soda. Separate <br />multiple entries with a semi-colon." , None))

        self.L1Index = self.paletteField(self.diaBox)
        self.L1Index.setGeometry(QtCore.QRect(80, 86, 144, 25))
        self.L1Index.setVerticalScrollBarPolicy(QtCore.Qt.ScrollBarAlwaysOff)
        self.L1Index.setObjectName("L1Index")
        self.L1Index.setToolTip(QtWidgets.QApplication.translate("DefinitionsManager", "primary language index terms", None))
        
        self.L2Index = self.paletteField(self.diaBox)
        self.L2Index.setGeometry(QtCore.QRect(338, 86, 144, 25))
        self.L2Index.setVerticalScrollBarPolicy(QtCore.Qt.ScrollBarAlwaysOff)
        self.L2Index.setObjectName("L2Index")
        self.L2Index.setToolTip(QtWidgets.QApplication.translate("DefinitionsManager", "secondary language index", None))
        ...
        self.L1.setToolTip(QtWidgets.QApplication.translate("DefinitionsManager", "definition in primary working language", None))

        self.L2 = self.paletteField(self.fieldBox)
        self.L2.setGeometry(QtCore.QRect(90, 233, 410, 70))
        self.L2.setObjectName("L2")
        self.L2.setToolTip(QtWidgets.QApplication.translate("DefinitionsManager", "definition in secondary working language", None))
        ...
        self.LnBox.setVerticalScrollBarPolicy(QtCore.Qt.ScrollBarAsNeeded)
    ...
    def formatBox(self):
        ...
        self.formatBox.setIcon(QtWidgets.QMessageBox.Warning)
        self.formatBox.setStandardButtons(QtWidgets.QMessageBox.Cancel)
        self.formatBox.setStandardButtons(QtWidgets.QMessageBox.Ok)
        self.formatBox.setDefaultButton(QtWidgets.QMessageBox.Ok)
            ...
            if len(self.Alternative.toPlainText()) !=0:
                ...
                for item in altList:
                    ...
                    if variant.endswith('.') == False:
                        variant += '.'
                    for item in dataIndex.root.iter('Lex'):
    ...
    def on_switchEgBtn_released(self):
        ...
        if self.switchEgDialog.exec_():
            ...
            try:
                ...
                self.badBox.setStandardButtons(QtWidgets.QMessageBox.Cancel)
